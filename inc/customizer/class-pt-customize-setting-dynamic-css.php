...
if ( class_exists( 'WP_Customize_Setting' ) && ! class_exists( 'BuildPress_Customize_Setting_Dynamic_CSS' ) ) {
	class BuildPress_Customize_Setting_Dynamic_CSS extends WP_Customize_Setting {
		...
		/**
		 * Active callback - should be CSS be printed out or not, depending on the context.
		 *
		 * @access protected
		 * @var callable | null
		 */
		...
		public function render_css() {
			...
			foreach ( $this->get_css_map() as $css_prop_raw => $css_selectors ) {
				...
				// Skip this CSS output, if the active callback returns false.
				if ( is_callable( $this->active_callback ) && false === call_user_func( $this->active_callback, $this->value() ) ) {
					continue;
				}
		...
		public function filter_css_property( $css_property ) {
			...
			if ( 'font-family' === $out['css_prop'] && 'custom' === $out['value'] ) {
				$default_font = 'primary_font' === $this->id ? 'Source Sans Pro' : 'Montserrat';
				$out['value'] = buildpress_get_font( $this->id, $default_font );
			}
